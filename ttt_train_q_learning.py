...
class Train:
    ...
    def TrainOneRound(self, afterstate_num, alpha=.1):
        ...
        while not afterstate.is_terminal():
            beforestate_num = self.random_policy.move(afterstate.get_num())  # opponent makes a move
            ...
            if beforestate.is_terminal():
                ...
                if beforestate.turn == 1:
                     self.target_policy.v_dict[afterstate.get_num(
                     )] += alpha * (max(candidates) - self.target_policy.v_dict[afterstate.get_num()])
                else:
                     self.target_policy.v_dict[afterstate.get_num(
                     )] += alpha * (min(candidates) - self.target_policy.v_dict[afterstate.get_num()])                   
