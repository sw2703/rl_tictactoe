         ...
         for num in range(int('1' + '0' * 9, 3), int('2' * 10, 3) + 1):
           ...
           if state.is_terminal():
             ...
             if opponent_afterstate.is_terminal():
               ...
               vi_update = max([self.policy_1.v_dict[x] for x in s_prime_choices])
         ...
         if delta < theta:
           ...
           for num in range(int('1' + '0' * 9, 3), int('2' * 10, 3) + 1):
             ...
             if not s.is_terminal():
                  ...
                  v_dict_slice = {s:self.policy_1.v_dict[s] for s in afterstates}
                  old_move = self.policy_1.move_dict[num]
                  self.policy_1.move_dict[num] = max(v_dict_slice, key = v_dict_slice.get)
